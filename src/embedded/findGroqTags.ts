...
export function findGroqTags(source: string): EmbeddedQuery[] {
  ...
  return tagLocations.map((loc) => {
    ...
    const { sanitized, interpolationRanges } = sanitizeInterpolations(
      loc.content
    );
    const finalContent = isFragment(sanitized)
      ? wrapFragment(sanitized)
      : sanitized;
    return {
      content: loc.content,
...
export function isInsideGroqTag(source: string, position: Position): boolean {
  ...
  for (const tag of tags) {
    if (position.line >= tag.start.line && position.line <= tag.end.line) {
      if (
        position.line === tag.start.line &&
        position.character < tag.start.character
      ) {
        continue;
      }
      if (
        position.line === tag.end.line &&
        position.character > tag.end.character
      ) {
        continue;
      }
      return true;
...
export function getGroqTagAtPosition(
  ...
  for (const tag of tags) {
    if (
      ...
      if (
        position.line === tag.range.start.line &&
        position.character < tag.range.start.character
      ) {
        continue;
      }
      if (
        position.line === tag.range.end.line &&
        position.character > tag.range.end.character
      ) {
        continue;
      }
      return tag;
