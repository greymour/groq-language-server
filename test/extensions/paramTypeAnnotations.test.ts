...
fn getAuthor($ref) = $ref-> { name }`;
      ...
      expect(annotation?.range.endIndex).toBe(source.indexOf('author') + 'author'.length);
    });
...
fn getAuthor($ref) = $ref-> { name }`;
      ...
      paramTypeAnnotationsExtension.hooks.onFunctionExtracted!(funcDef, source, funcStartIndex);
  ...
  describe('ExtensionRegistry integration', () => {
    ...
    it('getHook returns the hook when enabled', () => {
      ...
      expect(hooks[0].hook).toBe(paramTypeAnnotationsExtension.hooks.onFunctionExtracted);
    });
