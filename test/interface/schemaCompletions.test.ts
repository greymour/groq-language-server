...
describe('Schema-aware completions', () => {
  ...
  describe('document type completions', () => {
    it('suggests document types after _type ==', () => {
      ...
      const completions = getAutocompleteSuggestions(query, result.tree.rootNode, { line: 0, character: query.length }, schemaLoader);
...
describe('Schema-aware hover', () => {
  ...
  it('shows schema field info on hover', () => {
    ...
    const result = parser.parse(query);
    const hover = getHoverInformation(query, result.tree.rootNode, { line: 0, character: 22 }, schemaLoader);
  ...
  it('falls back to static info without schema', () => {
    ...
    const result = parser.parse(query);
    const hover = getHoverInformation(query, result.tree.rootNode, { line: 0, character: 22 });
